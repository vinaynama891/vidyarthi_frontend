...
const AdminDashboard = () => {
  ...
  // --- DOWNLOAD STUDENT LIST DROPDOWN & PDF GENERATOR ---
  ...
  const handleDownloadStudentListPDF = async (selectedClass) => {
    let listToExport = students;
    if (selectedClass && selectedClass !== 'All Classes') {
      listToExport = students.filter(s => s.class === selectedClass);
    }

    if (!listToExport || listToExport.length === 0) {
      showToast(`No student records found for ${selectedClass || 'All Classes'}`, 'error');
      setIsDownloadDropdownOpen(false);
      return;
    }
    ...
    const titleClass = (selectedClass && selectedClass !== 'All Classes') ? selectedClass : 'All Classes';
    const todayStr = new Date().toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' });

    // Draw Header on Every Page
    const drawHeader = () => {
      // 1. Vidyarthi Logo on Top Left
      if (logoImg) {
        doc.addImage(logoImg, 'PNG', 14, 8, 16, 16);
      }

      // 2. Centered Institute Name
      doc.setFontSize(16);
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(30, 41, 59);
      doc.text('VIDYARTHI CLASSES KOTA', pageWidth / 2, 16, { align: 'center' });

      // Subtitle
      doc.setFontSize(10);
      doc.setFont('helvetica', 'normal');
      doc.setTextColor(71, 85, 105);
      doc.text(`Student List - ${titleClass}  |  Date: ${todayStr}  |  Total Students: ${listToExport.length}`, pageWidth / 2, 23, { align: 'center' });
    ...
    const tableColumn = ["S.No.", "Student ID", "Student Name", "Father's Name", "Class", "Total Fee", "Paid Fee", "Pending Fee"];
    const tableRows = listToExport.map((st, index) => {
      const total = Number(st.totalFees) || 0;
      const paid = Number(st.paidFees) || 0;
    ...
    const fileName = `Student_List_${titleClass.replace(/\s+/g, '_')}_${new Date().toISOString().slice(0, 10)}.pdf`;
    doc.save(fileName);
    setIsDownloadDropdownOpen(false);
    showToast(`Downloaded student list PDF for ${titleClass}`, 'success');
  };

  // --- SEND WHATSAPP STUDENT SUMMARY TEMPLATE ---
  const handleSendStudentWhatsAppSummary = async (student) => {
    ...
    try {
      if (student.studentId) {
        const history = await apiFetch(`/api/attendance/history/student/${student.studentId}`);
        if (Array.isArray(history)) {
          const currentMonthLogs = history.filter(h => h.date && h.date.startsWith(currentMonthStr));
          presentDays = currentMonthLogs.filter(h => h.status === 'present').length;
          absentDays = currentMonthLogs.filter(h => h.status === 'absent').length;
        }
      }
    } catch (err) {
...
📅 *ATTENDANCE (${monthName.toUpperCase()}):*
                ...
                <div className="flex flex-wrap items-center gap-3 shrink-0">
                  ...
                  <div className="relative" ref={downloadDropdownRef}>
                    <button
                      ...
                      className="px-4 py-3 text-sm font-bold text-slate-700 bg-white border border-slate-200 hover:bg-slate-50 shadow-sm rounded-xl flex items-center gap-2 cursor-pointer transition-all"
                    >
                      <Download className="w-4 h-4 text-emerald-600" />
                      <span>Download List</span>
                      <ChevronDown className={`w-4 h-4 text-slate-400 transition-transform duration-200 ${isDownloadDropdownOpen ? 'rotate-180' : ''}`} />
                    </button>

                    {isDownloadDropdownOpen && (
                      <div className="absolute right-0 mt-2 w-56 bg-white border border-slate-100 rounded-xl shadow-xl z-50 py-2 max-h-72 overflow-y-auto">
                        <div className="px-3 py-1.5 text-[11px] font-bold text-slate-400 uppercase tracking-wider border-b border-slate-100 mb-1">
                          Select Class to Download PDF
                        </div>
                        <button
                          onClick={() => handleDownloadStudentListPDF('All Classes')}
                          className="w-full text-left px-4 py-2 text-xs font-semibold text-emerald-700 hover:bg-emerald-50 flex items-center justify-between transition-colors"
                        >
                        ...
                        {classesOptions.map((c) => {
                          ...
                          return (
                            <button
                              key={c}
                              onClick={() => handleDownloadStudentListPDF(c)}
                              className="w-full text-left px-4 py-2 text-xs text-slate-700 hover:bg-slate-50 flex items-center justify-between transition-colors"
                            >
                              <span>{c}</span>
                  ...
                  <button
                    ...
                    className="px-5 py-3 text-sm font-bold text-white bg-emerald-600 hover:bg-emerald-700 shadow-md hover:shadow-lg rounded-xl flex items-center gap-2 cursor-pointer"
                  >
                                  ...
                                  <div className="flex items-center gap-1.5">
                                    ...
                                    {student.phone && (
                                      <button
                                        ...
                                        className="text-emerald-500 hover:text-emerald-600 transition-colors cursor-pointer inline-flex items-center"
                                        title="Send Student Summary on WhatsApp"
                                      >
                                        <svg xmlns="http://www.w3.org/2000/svg" className="w-3.5 h-3.5 fill-current" viewBox="0 0 448 512">
                                          <path d="M380.9 97.1C339 55.1 283.2 32 223.9 32c-122.4 0-222 99.6-222 222 0 39.1 10.2 77.3 29.6 111L0 480l117.7-30.9c32.4 17.7 68.9 27 106.1 27h.1c122.3 0 224.1-99.6 224.1-222 0-59.3-25.2-115-67.1-157zm-157 341.6c-33.2 0-65.7-8.9-94-25.7l-6.7-4-69.8 18.3L72 359.2l-4.4-7c-18.5-29.4-28.2-63.3-28.2-98.2 0-101.7 82.8-184.5 184.6-184.5 49.3 0 95.6 19.2 130.4 54.1 34.8 34.9 56.2 81.2 56.1 130.5 0 101.8-84.9 184.6-186.6 184.6zm101.2-138.2c-5.5-2.8-32.8-16.2-37.9-18-5.1-1.9-8.8-2.8-12.5 2.8-3.7 5.6-14.3 18-17.6 21.8-3.2 3.7-6.5 4.2-12 1.4-32.6-16.3-54-29.1-75.5-66-5.7-9.8 5.7-9.1 16.3-30.3 1.8-3.7 .9-6.9-.5-9.7-1.4-2.8-12.5-30.1-17.1-41.2-4.5-10.8-9.1-9.3-12.5-9.5-3.2-.2-6.9-.2-10.6-.2-3.7 0-9.7 1.4-14.8 6.9-5.1 5.6-19.4 19-19.4 46.3 0 27.3 19.9 53.7 22.6 57.4 2.8 3.7 39.1 59.7 94.8 83.8 35.2 15.2 49 16.5 66.6 13.9 10.7-1.6 32.8-13.4 37.4-26.4 4.6-13 4.6-24.1 3.2-26.4-1.3-2.5-5-3.9-10.5-6.6z"/>
                                        </svg>
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="mt-2 pt-4 border-t border-dashed border-indigo-100 space-y-2">
                          ...
                          <div className="flex justify-end gap-2">
                            <button
                              onClick={() => {
                                let cleaned = enquiry.mobileNumber.replace(/\D/g, '');
                                if (cleaned.length === 10) {
                                  cleaned = '91' + cleaned;
                                }
                                window.open(`https://api.whatsapp.com/send?phone=${cleaned}`, '_blank');
                              }}
                              className="flex items-center gap-1.5 bg-emerald-600 hover:bg-emerald-700 text-white px-3 py-1.5 rounded-lg text-[11px] font-bold transition-all duration-200 cursor-pointer"
                            >
                              <svg xmlns="http://www.w3.org/2000/svg" className="w-3 h-3 fill-current" viewBox="0 0 448 512">
                                <path d="M380.9 97.1C339 55.1 283.2 32 223.9 32c-122.4 0-222 99.6-222 222 0 39.1 10.2 77.3 29.6 111L0 480l117.7-30.9c32.4 17.7 68.9 27 106.1 27h.1c122.3 0 224.1-99.6 224.1-222 0-59.3-25.2-115-67.1-157zm-157 341.6c-33.2 0-65.7-8.9-94-25.7l-6.7-4-69.8 18.3L72 359.2l-4.4-7c-18.5-29.4-28.2-63.3-28.2-98.2 0-101.7 82.8-184.5 184.6-184.5 49.3 0 95.6 19.2 130.4 54.1 34.8 34.9 56.2 81.2 56.1 130.5 0 101.8-84.9 184.6-186.6 184.6zm101.2-138.2c-5.5-2.8-32.8-16.2-37.9-18-5.1-1.9-8.8-2.8-12.5 2.8-3.7 5.6-14.3 18-17.6 21.8-3.2 3.7-6.5 4.2-12 1.4-32.6-16.3-54-29.1-75.5-66-5.7-9.8 5.7-9.1 16.3-30.3 1.8-3.7 .9-6.9-.5-9.7-1.4-2.8-12.5-30.1-17.1-41.2-4.5-10.8-9.1-9.3-12.5-9.5-3.2-.2-6.9-.2-10.6-.2-3.7 0-9.7 1.4-14.8 6.9-5.1 5.6-19.4 19-19.4 46.3 0 27.3 19.9 53.7 22.6 57.4 2.8 3.7 39.1 59.7 94.8 83.8 35.2 15.2 49 16.5 66.6 13.9 10.7-1.6 32.8-13.4 37.4-26.4 4.6-13 4.6-24.1 3.2-26.4-1.3-2.5-5-3.9-10.5-6.6z"/>
                              </svg>
                              Chat
                            </button>
